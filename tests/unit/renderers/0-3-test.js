...
import {
  createElement
} from 'mobiledoc-amp-html-renderer/utils/dom';
import Renderer from 'mobiledoc-amp-html-renderer';
import ImageCard from 'mobiledoc-amp-html-renderer/cards/image';
import {
  MARKUP_SECTION_TYPE,
  LIST_SECTION_TYPE,
  CARD_SECTION_TYPE,
  IMAGE_SECTION_TYPE
} from 'mobiledoc-amp-html-renderer/utils/section-types';

import {
  MARKUP_MARKER_TYPE,
  ATOM_MARKER_TYPE
} from 'mobiledoc-amp-html-renderer/utils/marker-types';
...
let renderer;
module('Unit: Mobiledoc AMP HTML Renderer - 0.3', {
  beforeEach() {
...
test('renders a mobiledoc with simple (no attributes) markup', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, '<p><b>hello world</b></p>');
});
...
test('renders a mobiledoc with complex (has attributes) markup', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, '<p><a href="http://google.com">hello world</a></p>');
});
...
test('renders a mobiledoc with multiple markups in a section', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, '<p><b>hello <i>brave new </i>world</b></p>');
});
...
test('renders a mobiledoc with image section', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, `<amp-img src="${dataUri}">`);
});
...
test('renders a mobiledoc with built-in image card', (assert) => {
  ...
  assert.equal(rendered, `<div><amp-img src="${dataUri}"></div>`);
});
...
test('renders a mobiledoc with card section', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, '<div>Howdy friend</div>');
});
...
test('rendering nested mobiledocs in cards', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, '<div><p>hello world</p></div>');
});
...
test('renders a mobiledoc with atom', (assert) => {
  ...
  let { result: rendered } = renderer.render(mobiledoc);
  assert.equal(rendered, '<p>Hello Bob</p>');
});
...
test('renders a mobiledoc with sectionElementRenderer', (assert) => {
  ...
  assert.equal(rendered, '<pre>hello world</pre><pre>hello world</pre><h2>hello world</h2>');
});
